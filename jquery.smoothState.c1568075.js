...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"wuFS":[function(require,module,exports) {
/**
...
(function ($, window, document, undefined) {
  ...
  utility = {
    ...
    shouldLoadAnchor: function shouldLoadAnchor($anchor, blacklist) {
      var href = $anchor.prop('href'); // URL will only be loaded if it's not an external link, hash, or blacklisted
  ...
  Smoothstate = function Smoothstate(element, options) {
    ...
    fetch = function fetch(request, callback) {
      ...
      var settings = utility.translate(request); // Don't prefetch if we have the content already or if it's a form
      ...
      cache = utility.clearIfOverCapacity(cache, options.cacheLength); // Let other parts of the code know we're working on getting the content
    ...
    clickAnchor = function clickAnchor(event) {
      var $anchor = $(event.currentTarget); // Ctrl (or Cmd) + click must open a new tab

      if (!event.metaKey && !event.ctrlKey && utility.shouldLoadAnchor($anchor, options.blacklist)) {
        var request = utility.translate($anchor.prop('href')); // stopPropagation so that event doesn't fire on parent containers.
